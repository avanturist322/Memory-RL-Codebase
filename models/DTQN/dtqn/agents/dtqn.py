...
class DtqnAgent(DqnAgent):
    def __init__(
        ...
        if isinstance(self.env_obs_length, tuple):
            self.obs_context[self.timestep] = self.env.reset().copy()
        else:
    ...
    def prepopulate(self, prepop_steps: int) -> None:
        ...
        for _ in range(prepop_steps):
            ...
            if isinstance(self.env_obs_length, tuple): # images
                pass
            else:
    ...
    def evaluate(self, n_episode: int = 10, eval_seeds = None, render: bool = False, videos_limit = 3, videos_dir = '/opt/Memory-RL-Codebase/eval/Minigrid_Memory/DTQN', run_name = 'run_99999', run_type = 'non_specified_runs') -> None:
        ...
        for i in range(n_episode):
            ...
            if render and i < videos_limit:
                ...
                curr_reward = float(ep_reward)
                if not os.path.exists(videos_dir + f"/{run_type}/{run_name}"):
                    os.makedirs(videos_dir + f"/{run_type}/{run_name}")

                video_path = videos_dir + f"/{run_type}/{run_name}/{run_name}_seed={curr_seed}_reward={curr_reward:0.2}.mp4"

                clip.write_videofile(video_path, fps=2)

            if eval_seeds is not None:
                curr_seed = eval_seeds[i]
                print(f'Episode: {i}, seed: {curr_seed} Reward: {ep_reward}, Steps: {timestep} Mean reward: {total_reward / (i + 1)}, Mean steps: {total_steps / (i + 1)}')
