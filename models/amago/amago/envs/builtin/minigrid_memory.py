...
from environments.Minigrid_Memory.env.env_minigrid import create_env
...
class MinigridMemoryGymEnv(GymEnv):
    def __init__(self, env_name: str, horizon: int, length: int):
        try:
            from environments.Minigrid_Memory.env.env_minigrid import create_env
        except ImportError:
            msg = "minigrid memory required"
            print(msg)
            exit()
