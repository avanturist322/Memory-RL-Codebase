...
class MysteryPathGymEnv(GymEnv):
    def __init__(self, env_name: str, horizon: int):
        try:
            from environments.Mystery_Path.env.env_mystery_path import create_env
        except ImportError:
            msg = "mystery path required"
            print(msg)
            exit()

        env = create_env(env_name) # "MysteryPath-Grid-v0"
