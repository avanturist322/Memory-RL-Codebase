...
class FrameProcessing(gym.Wrapper):
    ...
    def reset(self, seed, options):
        ...
        # obs = np.repeat(np.expand_dims(obs, 0), 3, 0)
        # print(obs.keys())
        # self._last_frame = obs.copy()

        return obs, {'key1': None}
...
class KeyToDoorAsGym(gym.Wrapper):
    def __init__(self, env):
        self.env = env
        self.action_space = gym.spaces.Discrete(4)
        self.observation_space = gym.spaces.Box(0.0, 255.0, (7,7), dtype=np.uint8)
...
class KeyToDoorGymEnv(GymEnv):
    def __init__(self, env_name: str, horizon: int, episode_timeout: int = 120):
        try:
            from environments.Key_To_Door.env import key_door_env
        except ImportError:
            msg = "key-to-door env. required"
            print(msg)
            exit()

        # env_name_2 = {'type': 'Minigrid', 
        #               'name': env_name} # 'MiniGrid-MemoryS13Random-v0'
        # LENGTH = length # 21 grid size

        # env = create_env(env_name_2, LENGTH) # "MysteryPath-Grid-v0"
