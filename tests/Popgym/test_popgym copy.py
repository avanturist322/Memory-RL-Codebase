    ...
    * Concentration
    ...
    def __init__(self, env_name, reset_params = None, realtime_mode = False) -> None:
        ...
        if isinstance(self._env.observation_space, (gymnasium.spaces.box.Box, gym.spaces.box.Box)):
            ...
            self._env.observation_space.obs_shape = (len(self._env.observation_space.spaces),)#tuple([1 for obs_space in self._env.observation_space.spaces])
            self._env.observation_space.obs_type = 'multidiscrete' 
        elif isinstance(self._env.observation_space, (gymnasium.spaces.MultiDiscrete, gym.spaces.MultiDiscrete)):
            # multi discrete observation - like discrete vector 
            self._env.observation_space.obs_shape = self.observation_space.shape 
            self._env.observation_space.obs_type = 'multidiscrete'
        ...
        try:
            ...
            # self.max_episode_steps = -1 # define 
...
# env = POPGymWrapper(env_name = 'ConcentrationEasy') # AutoencodeEasy
# print(env.reset())
# for i in range(3):
#     action = env.action_space.sample()
...
env = popgym.envs.autoencode.AutoencodeMedium()
#env = popgym.envs.concentration.ConcentrationMedium()
num_steps = 207

#print(env.observation_space.nvec)

# print([el.n for el in env.observation_space.spaces])
...
for i in range(num_steps):
    ...
    action = env.action_space.sample()
    obs, reward, terminated, truncated, info = env.step(action)
    print(obs, terminated, truncated,type(obs))
    print(action)
    # print(type(obs))
    # print(np.array(np.array(np.array([1, 2, 3]))))
    # wrapped = DiscreteAction(Flatten(PreviousAction(env))) # Append prev action to obs, flatten obs/action spaces, then map the multidiscrete action space to a single discrete action for Q learning
...
# env = POPGymWrapper('AutoencodeEasy')
